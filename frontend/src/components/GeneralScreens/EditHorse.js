...
const EditHorse = () => {
  ...
  useEffect(() => {
    const fetchHorse = async () => {
      try {
        const { data } = await api.get(`/horses/${slugParam}`);
        setForm({
          name: data.name || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await api.put(`/horses/${slugParam}`, fd, {
        headers: {
      ...
      setError(
        err?.response?.data?.message || "Unable to update horse. Please try again."
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <Wrapper>
      ...
      <form onSubmit={handleSubmit} className="form">
        ...
        <div className="grid-2">
          ...
          <div className="field">
            ...
            <select name="category" required value={form.category} onChange={handleChange}>
              ...
              <option value="At Risk / Urgent">At Risk / Urgent</option>
            </select>
          </div>
        </div>
        ...
        <div className="grid-3">
          <div className="field">
            <label>Sex</label>
            <select name="sex" required value={form.sex} onChange={handleChange}>
